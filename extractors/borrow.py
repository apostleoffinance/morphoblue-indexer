...
def normalize_borrow_event(event: dict[str, Any], chain_name: str) -> dict[str, Any]:
    """Normalize a raw Borrow event into a structured row."""
    ...
    return {
        ...
        "borrower": args.get("onBehalf") or args.get("borrower"),
...
def transform_borrow_events(events: list[dict[str, Any]], chain_name: str = "ethereum") -> list[dict[str, Any]]:
    """Transform raw event objects into normalized borrow rows."""
    return [normalize_borrow_event(event, chain_name) for event in events]
...
def fetch_borrow_events(
    ...
    """Fetch raw Borrow events for a named chain and enrich with block timestamp."""
    if chain_name not in CHAINS:
        raise ValueError(f"Unknown chain: {chain_name}")

    w3, contract = get_contract(chain_name)
    raw_events = contract.events.Borrow().get_logs(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fetch Borrow events from Morpho Blue contracts"
    )
    parser.add_argument(
        "--chains",
        nargs="+",
    ...
    for chain_name in args.chains:
        ...
        print(f"\n📍 Fetching Borrow events from {chain_name}...")
        try:
            raw_events = fetch_borrow_events(
                chain_name=chain_name,
                from_block=args.from_block,
                to_block=args.to_block,
            )
            events = transform_borrow_events(raw_events, chain_name=chain_name)
            print(f"✅ Found {len(events)} Borrow events on {chain_name}")
            if events:
                print(events[0])
