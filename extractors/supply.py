...
def normalize_supply_event(event: dict[str, Any], chain_name: str) -> dict[str, Any]:
    """Normalize a raw Supply event into a structured row."""
    ...
    return {
        ...
        "supplier": args.get("onBehalf") or args.get("supplier"),
...
def transform_supply_events(events: list[dict[str, Any]], chain_name: str = "ethereum") -> list[dict[str, Any]]:
    """Transform raw event objects into normalized supply rows."""
    return [normalize_supply_event(event, chain_name) for event in events]


def fetch_supply_events(
    chain_name: str,
    from_block: int,
    to_block: int,
) -> list[dict[str, Any]]:
    """Fetch raw Supply events for a named chain."""
    if chain_name not in CHAINS:
        raise ValueError(f"Unknown chain: {chain_name}")

    w3, contract = get_contract(chain_name)
    raw_events = contract.events.Supply().get_logs(
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Fetch Supply events from Morpho Blue contracts"
    )
    parser.add_argument(
        "--chains",
        nargs="+",
    ...
    for chain_name in args.chains:
        ...
        print(f"\n📍 Fetching Supply events from {chain_name}...")
        try:
            ...
            events = transform_supply_events(raw_events, chain_name=chain_name)
            print(f"✅ Found {len(events)} Supply events on {chain_name}")
            if events:
                print(events[0])
